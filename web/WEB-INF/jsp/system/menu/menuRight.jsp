    <%@ page language="java" contentType="text/html; charset=UTF-8"
        ...
        var tableIns;
        layui.extend({
        dtree:'<%=request.getContextPath()%>/resources/layui_ext/dist/dtree'
        ...
        ,{field:'title', title:'菜单名称',align:'center',width:'140'}
        ,{field:'href', title:'菜单地址',align:'center',width:'190'}
        ...
        // alert(params);
        tableIns.reload({
        url:"/SSM/menu/loadAllMenu?"+params
        })
        });
        ...
        if(layEvent === 'del'){ //删除
        //先判断当前菜单有没有子节点
        $.post("/SSM/menu/checkMenuHasChildren?id="+data.id,function(obj){
        if(obj.code>=0){
        layer.msg("当前菜单有子节点同，请先删子节点");
        }else{
        layer.confirm('真的删除【'+data.title+'】这个菜单吗', function(index){
        //向服务端发送删除指令
        $.post("/SSM/menu/deleteMenu",{id:data.id},function(res){
        layer.msg(res.msg);
        //刷新数据 表格
        tableIns.reload();
        //刷新左边的树
        ...
        $.post(url,obj.field,function(obj){
        layer.msg(obj.msg);
        //关闭弹出层
        ...
        url: "/SSM/menu/loadMenuManagerLeftTreeJson?spread=1", // 使用url加载（可与data加载同时存在）
        icon: "2",
        accordion:true
        ...
        tableIns.reload({
        url:"/SSM/menu/loadAllMenu?id="+id+"&page=1"
        })
        }
        </script>
        </body>
