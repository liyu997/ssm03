	...
	<script type="text/javascript">
	    ...
	    layui.extend({
			...
			$("#doSearch").click(function(){
				...
				tableIns.reload({
					url:"/SSM/role/loadAllRole?"+params
				})
			});
		   ...
		   table.on('tool(roleTable)', function(obj){
			  ...
			  if(layEvent === 'del'){ //删除
				  layer.confirm('真的删除【'+data.rolename+'】这个角色吗', function(index){
				       //向服务端发送删除指令
				       $.post("/SSM/role/deleteRole",{roleid:data.roleid},function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
			...
			function deleteBatch(){
			    ...
			    layer.confirm('真的删除选中的这些角色吗', function(index){
				       //向服务端发送删除指令
				       $.post("/SSM/role/deleteBatchRole",params,function(res){
				    	   layer.msg(res.msg);
				    	    //刷新数据 表格
							tableIns.reload();
				       })
			...
			function openselectRoleMenu(data){
				...
				mainIndex=layer.open({
					...
					yes:function(index, layero){
						...
						//保存角色和菜单的关系
						$.post("/SSM/role/saveRoleMenu",params,function(obj){
							layer.msg(obj.msg);
                            layer.close(mainIndex);
                        })
					},
					success:function(index){
					   ...
					   menuTree = dtree.render({
			              ...
			              checkbarData: "choose" ,
				          url: "/SSM/role/initRoleMenuTreeJson?roleid="+data.roleid // 使用url加载（可与data加载同时存在）
					    });
					}
				});
			}
